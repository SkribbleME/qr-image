var designAtt = require('./designAttributes');

function matrix2path(matrix, degreeOfRandom, dataRenderType) {
    var N = matrix.length;
    var filled = [];
    ...
    for (var row = 0; row < N; row++) {
        for (var col = 0; col < N; col++) {
            ...
            if (isDark(row, col)) {
                if (!isDark(row - 1, col)) {
                    path.push(plot(row, col, 'right', degreeOfRandom, dataRenderType));
                }
            } else {
                if (isDark(row, col - 1)) {
                    path.push(plot(row, col, 'down', degreeOfRandom, dataRenderType));
                }
            }
        }
    }
...
function pushSVGPath(matrix, stream, margin, dataRenderType) {
    matrix2path(matrix, 2, dataRenderType).forEach(function (subpath) {
        var res = '';
...
function designTemplate(matrix, eyeType, internalEyeColor, externalEyeColor, strokeWidth, strokeColour, text) {
    ...
    var res = '<path stroke-width="' + strokeWidth + '" fill="' + externalEyeColor + '" stroke-linecap="round" stroke="' + strokeColour + '" stroke-linejoin="round" d="'
    ...
    switch (eyeType) {
        case 'leaf':
            ...
            res += '<path stroke-width="' + strokeWidth + '" fill="' + internalEyeColor + '" stroke-linecap="round" stroke="' + strokeColour + '" stroke-linejoin="round" d="'
        ...
        case 'grid':
            ...
            res += '<circle cx="4.7" cy="4.7" r="2.5" fill="white"/><circle cx="4.7" cy="4.7" r="1.4" fill="' + internalEyeColor + '"/>'
            res += '<circle cx="' + (b - 4.7) + '" cy="4.7" r="2.5" fill="white"/><circle cx="' + (b - 4.7) + '" cy="4.7" r="1.4" fill="' + internalEyeColor + '"/>'
            res += '<circle cx="4.7" cy="' + (b - 4.7) + '" r="2.5" fill="white"/><circle cx="4.7" cy="' + (b - 4.7) + '" r="1.4" fill="' + internalEyeColor + '"/>'
            break;
        case 'box':
        default:
            res += 'M1 1 h7v7h-7z M2 2 v5h5v-5z '
            res += 'M' + (b-8) + ' 1 h7v7h-7z M' + (b-7) + ' 2 v5h5v-5z'
            res += 'M1 ' + (b-8) + ' h7v7h-7z M2 ' + (b-7) + ' v5h5v-5z" />'

            res += '<path stroke-width="' + strokeWidth + '" fill="' + internalEyeColor + '" stroke-linecap="round" stroke="' + strokeColour + '" stroke-linejoin="round" d="'
            res += 'M3 3 h3v3h-3z M'+ (b-6) + ' 3 h3v3h-3z M3 '+ (b-6) + ' h3v3h-3z" />'
    }
...
function SVG(matrix, stream, margin, size, qrOptions, xcor, ycor, text) {
    text = 'http://skrib.id/' + text.slice(text.lastIndexOf('/') + 1)

    externalEyeColor = qrOptions.externalEyeColor ? qrOptions.externalEyeColor : designAtt.SkribbleDefault.ExternalEye[0]
    internalEyeCol = qrOptions.internalEyeColor ? qrOptions.internalEyeColor : designAtt.SkribbleDefault.InternalEye[0]
    eyeStrokeCol = qrOptions.eyeStrokeColor ? qrOptions.eyeStrokeColor : designAtt.SkribbleDefault.EyeStroke[0]

    dataCol = qrOptions.dataColor ? qrOptions.dataColor : designAtt.SkribbleDefault.DarkColours[Math.floor(Math.random() * (designAtt.SkribbleDefault.DarkColours.length + 1))]
    dataStrokeCol = qrOptions.dataBorderColor ? qrOptions.dataBorderColor : designAtt.SkribbleDefault.Stroke[0]
    ...
    if (size > 0) {
        ...
        stream.push('width="100%" height="100%" ');
    }
    var xCor = XY * xcor
    var yCor = XY * ycor
    stream.push('x="' + xCor + '" y="' + yCor + '" ');
    stream.push('viewBox="0 0 ' + X + ' ' + X + '">');
    stream.push(designTemplate(matrix, qrOptions.eyeType, internalEyeCol, externalEyeColor, 0.1, eyeStrokeCol, text))
    stream.push(designDataTemplate(dataStrokeCol, dataCol, 0.2))
    pushSVGPath(matrix, stream, margin, qrOptions.dataRenderType);
    stream.push('"/>');
    stream.push('<text x="' + 1.25 + '" y="' + (X-1.2) + '" stroke="white" stroke-width="0.025" font-family="Verdana" font-size="0.7" text-anchor="start"> ' + text + ' </text>')
    stream.push('</svg>');
    stream.push(null);
}

...
function SVG(matrix, stream, margin, size, qrOptions, xcor, ycor, text) {
    ...
    internalEyeCol = qrOptions.internalEyeCol ? qrOptions.internalEyeCol : designAtt.SkribbleDefault.InternalEye[0]
    ...
    stream.push('"/>');
    stream.push('<text x="' + 1.25 + '" y="' + (X-1.2) + '" stroke="white" stroke-width="0.025" font-family="Verdana" font-size="0.7" text-anchor="start"> ' + 'skrib.id/' + text.slice(text.lastIndexOf('/')+1) + ' </text>')
    stream.push('</svg>');
    stream.push(null);
}

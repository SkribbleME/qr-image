...
function designTemplate(matrix, eyeType, eyeColour, eyeBorderColour, strokeWidth, strokeColour) {
...
function SVG(matrix, stream, margin, size) {
    ...
    stream.push(designTemplate(matrix, "grid", "#fd6d21", "#2e2979", 0.1,"#d5fd35"))
    stream.push(designDataTemplate('black', 'red', 0.2))
    pushSVGPath(matrix, stream, margin);
    stream.push('"/></svg>');
    stream.push(null);
}
